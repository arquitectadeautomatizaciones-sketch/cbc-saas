...
// ── 90-day consequence logic ─────────────────────────────
function calcular90dias(s: Selecciones, perdidaMensual: number): string[] {
  const perdida90 = perdidaMensual * 3
  const lineas: string[] = []

  if (perdida90 > 0) {
    lineas.push(
      `Pierdes $${perdida90.toLocaleString('en-US')} USD en prospectos que se enfrían solos — sin sistema que los reactive.`
    )
  }

  if ((s.q3 ?? 25) === 0) {
    lineas.push(
      'Sigues improvisando ante el "lo pienso" — esos leads terminan comprándole a quien sí hace seguimiento.'
    )
  } else if ((s.q3 ?? 25) < 25) {
    lineas.push(
      'Tu seguimiento en Excel o WhatsApp sigue perdiendo contexto — y cada vez que lo buscas, pierdes tiempo que otros usan para cerrar.'
    )
  }

  if ((s.q4 ?? 25) === 0) {
    lineas.push(
      'No mides tu tasa de cierre. Lo que no se mide, no mejora — y lo que no mejora, se estanca.'
    )
  } else if ((s.q4 ?? 25) < 25) {
    lineas.push(
      'Tu tasa de cierre sigue siendo una estimación. Sin el número exacto, no puedes mejorar lo que no ves.'
    )
  }

  if ((s.q5 ?? 25) === 0) {
    lineas.push(
      'Cada mensaje de seguimiento te sigue costando más de 20 minutos — tiempo que podrías usar en nuevos prospectos.'
    )
  }

  if (lineas.length < 3) {
    lineas.push(
      'Tus prospectos sin respuesta siguen acumulándose — hasta que eligen a tu competencia.'
    )
  }
...
function PantallaCarga({ nombre }: { nombre: string }) {
  ...
  const msgs = [
    `Analizando las respuestas de ${nombre || 'tu pipeline'}…`,
  ...
  useEffect(() => {
    const iv = setInterval(() => setMsg(m => Math.min(m + 1, msgs.length - 1)), 900)
  ...
  return (
    <div style={{ textAlign: 'center', padding: '60px 24px' }}>
      ...
      <div style={{
        ...
        border: `5px solid ${TEAL}30`,
        borderTopColor: TEAL,
        borderRadius: '50%',
        animation: 'spin 0.9s linear infinite',
      }} />
      <p style={{ margin: '0 0 10px', fontWeight: 800, fontSize: 18, color: VERDE }}>
        Calculando tu diagnóstico
      </p>
      <p style={{ margin: 0, fontSize: 14, color: '#6b7280', animation: 'pulse 1.8s ease-in-out infinite', minHeight: 22 }}>
        {msgs[msg]}
      </p>
...
export default function DiagnosticoInteractivo() {
  ...
  const listo = nombreTrimmed.length >= 2
    && sel.q1val !== null && sel.q2 !== null && sel.q3 !== null && sel.q4 !== null && sel.q5 !== null

  function verDiagnostico() {
    if (!listo) return
    setPaso('cargando')
    window.scrollTo({ top: (resultRef.current?.offsetTop ?? 0) - 80, behavior: 'smooth' })
    setTimeout(() => {
      setPaso('resultados')
  ...
  const r = paso !== 'form' && paso !== 'cargando' ? calcular(sel) : null
  const nombre1 = nombreTrimmed || 'tu pipeline'
    ...
    <div id="diagnostico" style={{ maxWidth: 720, margin: '0 auto', padding: '0 16px' }}>
      ...
      {paso === 'form' && (
        <div>
          ...
          {/* Campo nombre */}
          <div style={{ marginBottom: 32, background: 'white', borderRadius: 16, padding: '22px 22px 18px', border: `2px solid ${TEAL}40` }}>
            <label style={{ display: 'block', fontWeight: 700, fontSize: 16, color: VERDE, marginBottom: 8 }}>
              ¿Cómo te llamas?
            </label>
            <p style={{ margin: '0 0 12px', fontSize: 13, color: '#6b7280' }}>
              Usamos tu nombre para personalizar tu diagnóstico.
            </p>
            <input
              type="text"
              placeholder="Tu nombre o cómo te dicen en el trabajo"
              value={nombre}
              onChange={e => setNombre(e.target.value)}
              style={{
                display: 'block', width: '100%', padding: '12px 14px',
                borderRadius: 10, border: `2px solid ${nombreTrimmed.length >= 2 ? TEAL : '#e5e7eb'}`,
          ...
          <button
            ...
            style={{
              ...
              border: 'none', background: listo ? VERDE : '#d1d5db', color: 'white',
              fontSize: 16, fontWeight: 800, cursor: listo ? 'pointer' : 'not-allowed',
              transition: 'all 0.2s', letterSpacing: '0.01em',
            }}
          >
            {!listo
              ? (!nombreTrimmed || nombreTrimmed.length < 2
                  ? 'Escribe tu nombre para continuar'
                  : 'Responde todas las preguntas para continuar')
              : `Ver el diagnóstico de ${nombreTrimmed} →`}
          </button>
        </div>
      )}
      ...
      {paso === 'cargando' && (
        <div ref={resultRef}>
          <PantallaCarga nombre={nombreTrimmed} />
        </div>
      )}
      ...
      {(paso === 'resultados' || paso === 'desbloqueado') && r && (
        <div ref={resultRef}>
          ...
          {/* Score + gauge */}
          <div style={{ background: 'white', borderRadius: 20, padding: '32px 28px', boxShadow: '0 4px 24px rgba(0,0,0,0.08)', marginBottom: 20 }}>
            <div style={{ textAlign: 'center', marginBottom: 8 }}>
              <span style={{ fontSize: 12, fontWeight: 700, letterSpacing: '0.08em', color: '#9ca3af', textTransform: 'uppercase' }}>
          ...
          {/* Cuello de botella */}
          <div style={{ background: `${r.nivelColor}12`, border: `2px solid ${r.nivelColor}40`, borderRadius: 16, padding: '20px 22px', marginBottom: 20 }}>
            <p style={{ margin: '0 0 6px', fontSize: 12, fontWeight: 700, color: r.nivelColor, textTransform: 'uppercase', letterSpacing: '0.06em' }}>
              {nombreTrimmed ? `${nombreTrimmed}, tu cuello de botella hoy es: ${r.cuelloLabel}` : `Tu cuello de botella hoy: ${r.cuelloLabel}`}
            </p>
            <p style={{ margin: 0, fontSize: 15, color: VERDE, fontWeight: 600, lineHeight: 1.5 }}>{r.cuelloTexto}</p>
          </div>

          {/* Cita de autoridad — borde amarillo */}
          <div style={{ borderLeft: `4px solid ${AMARILLO}`, background: '#fffbeb', borderRadius: '0 12px 12px 0', padding: '16px 20px', marginBottom: 20 }}>
            <p style={{ margin: '0 0 6px', fontSize: 14, color: '#374151', lineHeight: 1.65, fontStyle: 'italic' }}>
              "Si llevas 10 años haciendo lo mismo, no tienes 10 años de experiencia. Tienes 1 año repetido 10 veces."
            </p>
            <p style={{ margin: 0, fontSize: 12, fontWeight: 700, color: '#92400e' }}>— Atribuido a John C. Maxwell</p>
          </div>

          {/* ROI mensual */}
          {r.perdidaMensual > 0 && (
            <div style={{ background: '#fef2f2', border: '2px solid #fecaca', borderRadius: 16, padding: '18px 22px', marginBottom: 20, display: 'flex', gap: 14, alignItems: 'center' }}>
              ...
              <div>
                ...
                <p style={{ margin: 0, fontSize: 15, color: '#7f1d1d', fontWeight: 600 }}>
                  Estás dejando aprox. <strong>${r.perdidaMensual.toLocaleString('en-US')} USD</strong> en la mesa cada mes por prospectos que se enfrían antes de que actúes.
                </p>
              </div>
            </div>
          )}
          ...
          {/* Bloque 90 días */}
          <div style={{ background: '#111827', borderRadius: 16, padding: '24px 22px', marginBottom: 20 }}>
            <p style={{ margin: '0 0 16px', fontSize: 13, fontWeight: 700, color: AMARILLO, textTransform: 'uppercase', letterSpacing: '0.06em' }}>
            ...
            {calcular90dias(sel, r.perdidaMensual).map((linea, i) => (
              <div key={i} style={{ display: 'flex', gap: 12, alignItems: 'flex-start', marginBottom: i < 2 ? 12 : 0 }}>
                <span style={{ color: AMARILLO, fontSize: 16, flexShrink: 0, marginTop: 1 }}>→</span>
                <p style={{ margin: 0, fontSize: 14, color: 'rgba(255,255,255,0.85)', lineHeight: 1.6 }}>{linea}</p>
              </div>
            ))}
          </div>

          {/* Fortalezas */}
          <div style={{ background: 'white', borderRadius: 16, padding: '24px 22px', boxShadow: '0 2px 12px rgba(0,0,0,0.06)', marginBottom: 20 }}>
          ...
          {/* PAYWALL */}
          {paso === 'resultados' && (
            <div style={{ background: VERDE, borderRadius: 20, padding: '28px 24px', textAlign: 'center', marginBottom: 20 }}>
              <div style={{ fontSize: 28, marginBottom: 10 }}>🔒</div>
              <p style={{ margin: '0 0 6px', fontWeight: 800, fontSize: 20, color: 'white' }}>
              ...
              <input
                ...
                style={{
                  ...
                  fontSize: 15, marginBottom: emailError ? 6 : 12, outline: 'none', boxSizing: 'border-box',
                }}
              />
          ...
          {paso === 'desbloqueado' && (
            <>
              ...
              <div style={{ background: `linear-gradient(135deg, ${VERDE} 0%, #2d6b62 100%)`, borderRadius: 20, padding: '32px 24px', textAlign: 'center' }}>
                <p style={{ margin: '0 0 8px', fontSize: 12, fontWeight: 700, letterSpacing: '0.1em', color: TEAL, textTransform: 'uppercase' }}>
                  {nombreTrimmed ? `Hecho para ${nombreTrimmed}` : 'Hecho para ti'}
                </p>
                <p style={{ margin: '0 0 10px', fontWeight: 800, fontSize: 22, color: 'white', lineHeight: 1.3 }}>
                  CBC resuelve exactamente lo que acabas de diagnosticar
                </p>
                <p style={{ margin: '0 0 24px', fontSize: 14, color: 'rgba(255,255,255,0.75)', lineHeight: 1.6 }}>
                  Semáforo automático, seguimientos programados, mensajes personalizados y reporte listo en un toque. 7 días gratis. Sin tarjeta.
                </p>
                <a href="/empezar" style={{ display: 'inline-block', padding: '15px 32px', borderRadius: 12, background: TEAL, color: VERDE, fontWeight: 800, fontSize: 16, textDecoration: 'none' }}>
